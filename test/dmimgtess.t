...
do
  ...
  case ${testid} in
    # !!6
    test_tess ) test1_string="dmimgtess infile=$INDIR/peaks.fits outfile=$outfile minpix=0 edge=3 meth=tess  clob+"
            ;;
    test_tri ) test1_string="dmimgtess infile=$INDIR/peaks.fits outfile=$outfile minpix=0 edge=3 meth=tri  clob+"
            ;;
...
dmdiff $outfile"[region]" $savfile"[region]" tol=$SAVDIR/tolerance > /dev/null 2>>$LOGFILE
if  test $? -ne 0 ; then
  echo "ERROR: MISMATCH in $outfile[region]" >> $LOGFILE
  mismatch=0
fi
